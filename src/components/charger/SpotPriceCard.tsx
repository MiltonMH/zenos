...
export function SpotPriceCard({ currentPrice, trend, nextHourPrice, optimalTime }: SpotPriceCardProps) {
  ...
  const priceLevelConfig = {
    low: { label: "Lågt pris", color: "text-success", bg: "bg-success/20" },
    medium: { label: "Normalt pris", color: "text-warning", bg: "bg-warning/20" },
    high: { label: "Högt pris", color: "text-destructive", bg: "bg-destructive/20" },
  };

  return (
    <GlassCard>
      <div className="flex items-start justify-between mb-4">
        <h3 className="text-sm font-medium text-muted-foreground">Elpris just nu</h3>
        <span className={cn(
      ...
      <div className="flex items-baseline gap-2 mb-4">
        ...
        <span className="text-muted-foreground">SEK/kWh</span>
        <div className={cn("flex items-center gap-1 ml-2", trendColor)}>
          <TrendIcon className="w-4 h-4" />
          <span className="text-sm">
            {trend === "up" ? "Stigande" : trend === "down" ? "Fallande" : "Stabilt"}
          </span>
        </div>
      </div>
      ...
      <div className="flex justify-between text-sm">
        <div>
          <span className="text-muted-foreground">Nästa timme: </span>
          <span className={cn(
            nextHourPrice > currentPrice ? "text-destructive" : "text-success"
          )}>
            {nextHourPrice.toFixed(2)} SEK
          </span>
        </div>
        {optimalTime && (
          <div>
            <span className="text-muted-foreground">Bäst tid: </span>
